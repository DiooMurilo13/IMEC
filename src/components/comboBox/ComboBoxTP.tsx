...
function ComboModalTP({ option, value, onPress, defaultValue }: Form) {
  ...
  const handleSearch = (nome) => {
    const usuariosFiltradosa = obj.filter((usuario) =>
      usuario.tipoProduto_descricao.toLowerCase().includes(nome.toLowerCase())
    );
    setObjFiltrado(
        ...
        <Button
          ...
          funcao={
            <Text className="text-2xl font-bold text-myColor-100">
              {value
                ? value.tipoProduto_descricao
                : `Selecione o ${defaultValue}`}
            </Text>
          }
        />
        <Modal
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <CardScreenTP
                  ...
                  descricao={item.tipoProduto_descricao}
